...
def block():
    while True:
        ...
        if t.weekday() == 5:
            ...
            if i==1:
                print("B-Blocktime... Already Blocked")
                time.sleep(300)
            else:
                print("B-Blocktime... Blocking")
...
def unblock():
    while True:
        ...
        if t.weekday() == 5:
            ...
            i=wreg.QueryValueEx(key, "DisallowRun")[0]
            if i==0:
                print("U-Weekday Not blocked",future)
                time.sleep((future-t).total_seconds())
                continue
            else:
                print("U-Weekday blocked... Unblocking",future)
            ...
            i=wreg.QueryValueEx(key, "DisallowRun")[0]
            if i==0:
                print("U-UnBlocktime... Already UnBlocked")
                time.sleep(300)
                continue
            else:
                print("U-UnBlocktime... Unblocking")
                wreg.SetValueEx(key,"DisallowRun",0,wreg.REG_DWORD,0)
                call('taskkill /F /IM explorer.exe & start explorer', shell=True)
                time.sleep(300)
                continue
        else:
            print("U-Not UnBlocking time...")
            time.sleep(300)
    return None
